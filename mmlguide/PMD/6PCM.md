...
- P86DRV: uses the PC98's 86 soundboard's PCM as an alternate to the ADPCM
...
Once the DOS folder is setup, get the following files:
...
You'll also need an audio editor that can change sample rates and export unsigned 8-bit audio. For this tutorial I'll use Audacity, but I only recommend using versions up to 2.4.2 due to data colection being implemented from 3.0 onwards. You can get it from [here](https://www.fosshub.com/Audacity-old.html).
...
- Resample them to below 55khz
    ...
    The OPNA stores its samples at a max of 55khz, so. 16khz samples will play at `o5g`, giving a fair amount of upwards range.

- Apply a filter for cymbal-type instruments

    Cymbals and hihats tend to generate a lot of noise during ADPCM conversion, so I recommend applying the filter to avoid it happening.
...
- Export them as 16-bit unsigned RAW.
    ...
    In audacity this is done by going to "Export Audio..." choosing "Other uncompressed files" at the time of export and then choosing "16-bit unsigned" and RAW (header-less).


I also recommend letting a brief moment of silence after the sample finishes to avoid potential clicks.
...
`/w` determines the (input) file is a RAW 16-bit little-endian file, `/a` determines the (output) file is ADPCM.
...
1. Run the DOS driver to enable use of pmdpcm:
...
2. Run the following command to clear the RAM:
    ```
    pmdpcm /c
    ```
...
3. For every .spb sample you want to include in the file, run:
    ...
    ```
    In which `<id>` correspond to the instrument number to be called with the `@` command. Range is 0-255.

    This will load the sample into the ram and associate its addresses to its ID.
...
The PPC file needs to always be in the same folder as the .MML or .M file in order for the song to be played properly. Some players won't play the song at all if the intended sample bank is not present or pre-loaded in memory.

To sequence the ADPCM we'll use the `J` channel.

#### Set instrument (`@<value>`)

To choose the PCM isntrument, use `@`, in which the value corresponds to the sample ID in the PPC. This is will make the sample play as a whole once.
...
However, in ADPCM's case is a complicated process because of its encoding format.

Firstly the step count doesn't correspond to the same as the original sample. One way I found to get a rough estimate of the loop points was to divide the sample position Audacity gives by 64 and use this value. However this is based purely on trial and error, and there's no documentation on this.

Secondly, ADPCM doesn't natively support loop points, instead looping the entire sample. PMD circumvent this by changing the sample start offset, but due to how the sample data is interpreted, this changes the actual waveform output, making it dificult to make a perfectly looping sample and sometimes lowering the volume.

#### Notes

As mentioned before, if the sample is encoded as 16000hz, `o5g` will play the same as the source file.
...
- Resample them
    ...
    As PMD tries to make P86DRV compatible with ADPCM, it'll have many similarities with it. Including its recommended sample rates and whatnot.

    The only documented sample rate is 16504hz, which will play at `o5g`. But since some audio editor don't support it, I recommend exporting to the closest one upwards. I use 22050hz.
...
`/w` determines the (input) file is a RAW 16-bit little-endian file, `/b` determines the (output) file is 8-bit signed.
...
1. Run the following 2 DOS drivers to enable use of pmdpcm:
...
2. Run the following command to clear the sample memory:
    ```
    pmdpcm86 /c
    ```
...
The P86 file needs to always be in the same folder as the .MML or .M file in order for the song to be played properly. Some players won't play the song at all if the intended sample bank is not present or pre-loaded in memory.

To sequence the 86PCM we'll use the `J` channel.
...
To get the correct points, I search for them in samples in audacity and then divide the value by 32.
